...
class CoinDataManager:
    ...
    def get_coin_features(self, start, end, period=300, features=('close',)):
        ...
        self._coins = coins
        #for coin in coins:      #Change21
        #    self._update_data(start, end, coin)   #Change22
    ...
    def _fill_part_data(self, start, end, coin, cursor):
        ...
        for c in chart:
           # if c["date"] > 0: #Change11
           if float(c["date"]) > 0:
                #if c['weightedAverage'] == 0:  #Change12
                if float(c['weightedAverage']) == 0:
                    #weightedAverage = c['close']  #Change13
                    weightedAverage = float(c['close'])
                else:
                    #weightedAverage = c['weightedAverage'] #Change14
                    weightedAverage = float(c['weightedAverage'])

                # NOTE here the USDT is in reversed order
                if 'reversed_' in coin:
                    cursor.execute(
                        #'INSERT INTO History VALUES (?,?,?,?,?,?,?,?,?)',   #Change19
                        ...
                        #(c['date'], coin, 1.0 / c['low'], 1.0 / c['high'],  #Change15
                         #1.0 / c['open'],                                   #Change16
                         #1.0 / c['close'], c['quoteVolume'], c['volume'],   #Change17
                         #1.0 / weightedAverage))                            #Change18
                else:
                    cursor.execute(
                        #'INSERT INTO History VALUES (?,?,?,?,?,?,?,?,?)',   #Change20
                        'INSERT OR IGNORE INTO History VALUES (?,?,?,?,?,?,?,?,?)',
